<template>
  <div id="app">
    ...
    <div>
        <center>
        <label for="select_name">查询歌曲类型</label><input type="text" placeholder="请输入歌曲类型" id="select_type" class="form-control" v-model="mymusic.musictype"><br>
        <button @click="findByType()" class="btn btn-primary">查询</button><br><br>
        <button @click="getmusic()" class="btn btn-primary">查询所有</button><br>
...
export default {
  ...
  methods:{
      ...
      deleteById(){
          Axios.delete("http://localhost:8080/musics/"+this.mymusic.id)
          .then(this.getmusic)
      }
  }
}
